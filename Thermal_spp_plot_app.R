...
Data <- readRDS(paste0(directorio, "muestreo_aleat_TA_TC_TT_percent_0.001.RDS")) # RUTA A LOS DATOS
...
server <- function(input, output, session) {
  ...
  output$trendPlot <- renderPlot({
    ...
    ggplot() +
      geom_smooth(data = Data, aes(x = Año_Mes, y = TMIN, color = "TMIN - General"), method = "lm", se = FALSE) +
      geom_smooth(data = filtered_data, aes(x = Año_Mes, y = TMIN, color = "TMIN - Specie"), method = "lm", se = FALSE) +
      geom_smooth(data = Data, aes(x = Año_Mes, y = TMAX, color = "TMAX - General"), method = "lm", se = FALSE) +
      geom_smooth(data = filtered_data, aes(x = Año_Mes, y = TMAX, color = "TMAX - Specie"), method = "lm", se = FALSE) +
      ggtitle(paste0(input$selected_species)) +
      labs(x = "Year", y = "ºC", color = "Legend", fill = "Legend") +
      scale_color_manual(values = c("TMIN - General" = "dodgerblue", 
                                    "TMIN - Specie" = "dodgerblue4", 
                                    "TMAX - General" = "firebrick1", 
                                    "TMAX - Specie" = "firebrick4")) +
      theme_minimal()
  })
}
